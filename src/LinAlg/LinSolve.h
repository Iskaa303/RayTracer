...
/* *************************************************************************************************
	...
	*** INPUTS ***
	...
	aMatrix		Matrix2<T>		The matrix of independent variables (X in the above equation).
...
template <typename T>
int LinSolve(const Matrix2<T> &aMatrix, const Vector<T> &bVector, Vector<T> &resultVec)
{
	...
	// Use this to create a Matrix2 object with the same data (nx1).
	Matrix2<T> bMatrix(numDims, 1, bVecData);
	
	// Combine the two matrices together.
	inputMatrix.Join(bMatrix);
	
	/* Use Gaussian elmination to convert to row-echelon form. */
	Matrix2<T> rowEchelonMatrix = inputMatrix.RowEchelon();
